...
int main(int argc, char **argv) {
  ...
  const std::string instr_inst_file_name(sourcePath / "vector_vector_add_insts.txt");
  const std::string pdi_file_name(sourcePath / "vector_vector_add.pdi");
  ...
  std::vector<uint32_t *> input1(num_pkts);
  std::vector<uint32_t *> input2(num_pkts);
  ...
  for (int pkt_iter = 0; pkt_iter < num_pkts; pkt_iter++) {
    r = hsa_amd_memory_pool_allocate(global_kernarg_mem_pool, data_buffer_size, 0,
                                     reinterpret_cast<void **>(&input1[pkt_iter]));
    assert(r == HSA_STATUS_SUCCESS);

    r = hsa_amd_memory_pool_allocate(global_kernarg_mem_pool, data_buffer_size, 0,
                                     reinterpret_cast<void **>(&input2[pkt_iter]));
    assert(r == HSA_STATUS_SUCCESS);
    ...
    for (std::size_t i = 0; i < num_data_elements; i++) {
      *(input1[pkt_iter] + i) = i * (pkt_iter + 1);
      *(input2[pkt_iter] + i) = i * (pkt_iter + 1) + 3;
    ...
    cmd_payload->data[5] = LOW_ADDR(input1[pkt_iter]);
    cmd_payload->data[6] = HIGH_ADDR(input1[pkt_iter]);
    cmd_payload->data[7] = LOW_ADDR(input2[pkt_iter]);
    cmd_payload->data[8] = HIGH_ADDR(input2[pkt_iter]);
    cmd_payload->data[9] = LOW_ADDR(output[pkt_iter]);
    cmd_payload->data[10] = HIGH_ADDR(output[pkt_iter]);
    cmd_payload->data[11] = num_data_elements * sizeof(uint32_t);
    cmd_payload->data[12] = num_data_elements * sizeof(uint32_t);
    cmd_payload->data[13] = num_data_elements * sizeof(uint32_t);
  ...
  for (int pkt_iter = 0; pkt_iter < num_pkts; pkt_iter++) {
    for (std::size_t i = 0; i < num_data_elements; i++) {
      const auto expected = *(input1[pkt_iter] + i) + *(input2[pkt_iter] + i);
      const auto result = *(output[pkt_iter] + i);

      std::cout << "Input1: " << *(input1[pkt_iter] + i) << std::endl;
      std::cout << "Input2: " << *(input2[pkt_iter] + i) << std::endl;
    ...
    assert(r == HSA_STATUS_SUCCESS);
    r = hsa_amd_memory_pool_free(input1[pkt_iter]);
    assert(r == HSA_STATUS_SUCCESS);
    r = hsa_amd_memory_pool_free(input2[pkt_iter]);
    assert(r == HSA_STATUS_SUCCESS);
